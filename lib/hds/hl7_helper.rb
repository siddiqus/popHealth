class HL7Helper < HealthDataStandards::Util::HL7Helper
  ...
  def self.timestamp_to_integer(timestamp)
    if timestamp && timestamp.length >= 4
      ...
      month = (timestamp.length >= 6 && timestamp[4..5].to_i != 0) ? timestamp[4..5].to_i : 1
      day = (timestamp.length >= 8 && timestamp[6..7].to_i != 0) ? timestamp[6..7].to_i : 1
      hour = (timestamp.length >= 10 && timestamp[8..9].to_i != 0) ? timestamp[8..9].to_i : 0
